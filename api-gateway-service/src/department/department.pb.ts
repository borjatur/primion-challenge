/* eslint-disable */
import { GrpcMethod, GrpcStreamMethod } from "@nestjs/microservices";
import { Observable } from "rxjs";

export const protobufPackage = "department";

export interface Empty {
}

export interface DepartmentAssignData {
  id: number;
  userId: number;
}

export interface DepartmentData {
  id: number;
  name: string;
  departmentAssign: DepartmentAssignData[];
}

export interface CreateDepartmentRequest {
  name: string;
}

export interface CreateDepartmentResponse {
  error: string[];
  data: DepartmentData | undefined;
}

export interface GetDepartmentByIdRequest {
  id: number;
}

export interface GetDepartmentByIdResponse {
  error: string[];
  data: DepartmentData | undefined;
}

export interface UpdateDepartmentByIdRequest {
  id: number;
  name?: string | undefined;
}

export interface UpdateDepartmentByIdResponse {
  error: string[];
  data: DepartmentData | undefined;
}

export interface DeleteDepartmentByIdRequest {
  id: number;
}

export interface DeleteDepartmentByIdResponse {
  error: string[];
}

export interface GetAllDepartmentsResponse {
  error: string[];
  data: DepartmentData[];
}

export const DEPARTMENT_PACKAGE_NAME = "department";

export interface DepartmentServiceClient {
  createDepartment(request: CreateDepartmentRequest): Observable<CreateDepartmentResponse>;

  getDepartmentById(request: GetDepartmentByIdRequest): Observable<GetDepartmentByIdResponse>;

  updateDepartmentById(request: UpdateDepartmentByIdRequest): Observable<UpdateDepartmentByIdResponse>;

  deleteDepartmentById(request: DeleteDepartmentByIdRequest): Observable<DeleteDepartmentByIdResponse>;

  getAllDepartments(request: Empty): Observable<GetAllDepartmentsResponse>;
}

export interface DepartmentServiceController {
  createDepartment(
    request: CreateDepartmentRequest,
  ): Promise<CreateDepartmentResponse> | Observable<CreateDepartmentResponse> | CreateDepartmentResponse;

  getDepartmentById(
    request: GetDepartmentByIdRequest,
  ): Promise<GetDepartmentByIdResponse> | Observable<GetDepartmentByIdResponse> | GetDepartmentByIdResponse;

  updateDepartmentById(
    request: UpdateDepartmentByIdRequest,
  ): Promise<UpdateDepartmentByIdResponse> | Observable<UpdateDepartmentByIdResponse> | UpdateDepartmentByIdResponse;

  deleteDepartmentById(
    request: DeleteDepartmentByIdRequest,
  ): Promise<DeleteDepartmentByIdResponse> | Observable<DeleteDepartmentByIdResponse> | DeleteDepartmentByIdResponse;

  getAllDepartments(
    request: Empty,
  ): Promise<GetAllDepartmentsResponse> | Observable<GetAllDepartmentsResponse> | GetAllDepartmentsResponse;
}

export function DepartmentServiceControllerMethods() {
  return function (constructor: Function) {
    const grpcMethods: string[] = [
      "createDepartment",
      "getDepartmentById",
      "updateDepartmentById",
      "deleteDepartmentById",
      "getAllDepartments",
    ];
    for (const method of grpcMethods) {
      const descriptor: any = Reflect.getOwnPropertyDescriptor(constructor.prototype, method);
      GrpcMethod("DepartmentService", method)(constructor.prototype[method], method, descriptor);
    }
    const grpcStreamMethods: string[] = [];
    for (const method of grpcStreamMethods) {
      const descriptor: any = Reflect.getOwnPropertyDescriptor(constructor.prototype, method);
      GrpcStreamMethod("DepartmentService", method)(constructor.prototype[method], method, descriptor);
    }
  };
}

export const DEPARTMENT_SERVICE_NAME = "DepartmentService";
